...
fn day04(lines: impl Iterator<Item = impl AsRef<str>>) -> u32 {
    let mut sum = 0;
    for line in lines {
...
impl Card {
    fn new(id: u32, win_nums: HashSet<u8>, your_nums: HashSet<u8>) -> Self {
        Self {
            id,
            win_nums,
            your_nums,
    ...
    fn get_score(self: &Self) -> u32 {
        let matches = self.win_nums.intersection(&self.your_nums).count() as u32;
        match matches {
            0 => 0,
            n => 2_u32.pow(n - 1),
        }
